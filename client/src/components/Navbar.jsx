...
const Navbar = () => {
  ...
  const linkConfig = {
    ...
    Workshop: { path: '/#workshops', sectionId: 'workshops' },
  ...
  // This useEffect handles the sliding animation for desktop
  useEffect(() => {
    ...
    if (activeItemElement && sliderElement) {
  ...
  useEffect(() => {
    if (location.pathname === '/') {
      ...
      switch (location.pathname) {
        case '/contact': setActiveLink('Contact Us'); break;
        case '/about': setActiveLink('About Us'); break;
        case '/workshop': setActiveLink('Workshop'); break;
        case '/teams': setActiveLink('Team'); break;
        default: setActiveLink(''); break;
      }
    }
  }, [location]);
  ...
  return (
    <>
      <nav ref={navRef} className="sticky top-[15px] z-50 mt-4 mb-16 h-12 flex items-center">
        ...
        {/* ▼ HORIZONTAL ALIGNMENT: 'justify-end' for right, 'justify-start' for left. Also change padding 'pr-4' to 'pl-4' for left. */}
        <div className="md:hidden flex-1 flex justify-end pr-4">
          <button
            onClick={toggleMobileMenu}
            aria-label="Toggle mobile menu"
            // ▼ SHAPE & STYLE: 'rounded-xl' for rounded square, 'rounded-full' for circle.
